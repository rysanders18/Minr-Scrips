// engine_standard.js — the rychess engine transplanted to STANDARD chess
// rules, for Elo calibration against normal engines (Stockfish). The search,
// branching caps, move ordering, qsearch, and evaluation are IDENTICAL to
// engine.js; only the variant pawn rules are removed:
//   - no lateral pawn moves/captures
//   - double push only from the home rank (never promotes)
//   - no lateral pawn attacks in isSquareAttacked
//   - promotion is still auto-queen for the engine's own move generation,
//     but makeMove/unmakeMove honor underpromotion codes 1-3 so externally
//     supplied (Stockfish) moves keep the boards in sync.
//
...
function genPawnMoves(st, sq, out) {
    ...
    if (rank !== backRank) {
        ...
        if (rank === homeRank && b[oneStep] === 0 && b[twoStep] === 0)
            out.push(enc(sq, twoStep, 0, 2));
    ...
    // (standard rules: no lateral pawn moves)
}
...
function isSquareAttacked(st, sq, byColor) {
    ...
    // (standard rules: no lateral pawn attacks)
...
function makeMove(st, move) {
    ...
    if (promo) placed = promo + 1 + (mColor === 1 ? 6 : 0); // 1=N..4=Q -> piece codes 2..5 / 8..11
    b[to] = placed;
