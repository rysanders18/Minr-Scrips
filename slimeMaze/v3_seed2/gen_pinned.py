...
#   slimemaze/fall.msc     - fall(player): respawn at a random splice
#                            window 30-60 blocks above the last
#                            bounced slime block (lastBlock); players
#                            who never bounced get a random one of the
#                            3 starts furthest from the fall point
...
SG_PART_LINES = 1900    # slimegrounds parts: the create lines carry a
                        ...
                        # part under the known-good ~300KB hastebin cap
...
FALL_SETBACK_MIN = 30
FALL_SETBACK_MAX = 60
# a fall respawns the player at a splice window between MIN and MAX
# blocks (vertically) above their last bounced slime block: MIN is the
# floor every fall has to clear, MAX caps how much progress a single
# fall can cost. Near the top of the maze nothing sits MIN higher, so
# falls there get the largest lift that exists instead. Only
# never-bounced players are sent back to a top start (see fall.msc)
NMS_VARS = [
...
def emit(sim, outdir):
    ...
    FUTURE_DEPTH = 3     # slimeblock() receives ALL downstream bounce
                         ...
                         # 2026-08-19, mirrored from HEAD): the old
                         # literal next1/next2 pair guessed the arm at
                         # forks and failed at intersections
    def future_blocks(i):
    ...
    # BLOCK (never the cell), `future` a Block[] of EVERY downstream
    # bounce block within FUTURE_DEPTH bounces (user 2026-08-19:
    # covers all arms at forks). Cells are deduped: nearest owner
    ...
    # EMIT_SERVER_SET (user 2026-08-17, mirrored from HEAD): only the
    # server import set is generated - grounds stays DECLARED (its
    # implementation lives on the server) but no grounds or remove*
    # files are written
    ...
    # fall handler: respawn at a random splice window between
    # FALL_SETBACK_MIN and FALL_SETBACK_MAX blocks (vertically) above
    # the player's last bounced slime block (lastBlock, set by the
    # slimeblock hook). Windows below the anchor are never picked (a
    # fall must not skip progress); an empty band falls back to the
    # closest window still clearing MIN, then - near the top, where
    # nothing sits MIN higher - to the highest window at or above the
    # anchor, so a fall never sends the player back to the top.
    # Never-bounced players (lastBlock == null) keep the old behavior:
    # a random one of the 3 starts furthest from the fall point.
    wins, seen = [], set()
    for sp in sim.splices:
        w = sim.blocks[sp['w0']]
        key = (w['x'], w['y'], w['z'])
    ...
    fl = [
        '# fall(Player player)',
        '# Run whenever a player falls off the maze: puts them back on the',
        '# route AT LEAST %d blocks (vertically) above their last bounced'
        % FALL_SETBACK_MIN,
        '# slime block (lastBlock, maintained by slimeblock()). Respawn',
        '# points are the %d unique SPLICE WINDOW destinations from' % nw,
        '# splices.txt - always real teleport landings on the route. A',
        '# random window with y in [anchor+%d, anchor+%d] is chosen'
        % (FALL_SETBACK_MIN, FALL_SETBACK_MAX),
        '# (never below the anchor - falling must not skip progress); if',
        '# that band is empty the closest window that still clears +%d is'
        % FALL_SETBACK_MIN,
        '# used, cap ignored, and if even that does not exist - near the',
        '# top, where nothing sits %d higher - the highest window at or'
        % FALL_SETBACK_MIN,
        '# above the anchor is used, the biggest setback the maze can',
        '# give. So a fall NEVER sends the player back to the top. The',
        '# lowest window is y=%d, the highest y=%d. Players who'
        % (min(w[1] for w in wins), max(w[1] for w in wins)),
        '# have never bounced (lastBlock == null) keep the old behavior: a',
        '# random pick among the 3 top starts furthest from where they',
        '# fell. The tp lands them 3 blocks above the window block,',
        '# centered, keeping their facing. Wire this to the map\'s',
        '# fall-catch region / kill floor. Effects live in fallFx1',
        '# (pre-tp) and fallFx2 (post-tp) - HAND-TUNED hooks that',
        '# regeneration never overwrites.',
        '',
        '@using %s' % NAMESPACE,
        '@fast',
        '',
        '# the %d unique splice window blocks (deduped from splices.txt;'
        % nw,
        '# parallel arrays - keep index-aligned)',
        '@define Int[] wxs = Int[%s]'
        % ', '.join(str(w[0]) for w in wins),
        '@define Int[] wys = Int[%s]'
        % ', '.join(str(w[1]) for w in wins),
        '@define Int[] wzs = Int[%s]'
        % ', '.join(str(w[2]) for w in wins),
        '',
        '# ---- never bounced: old top-start behavior ----------------------',
        '@if lastBlock[player] == null',
        '    @define BlockLocation ploc = '
        'player.getLocation().asBlockLocation()',
    ...
    fl += [
        ...
        '    @done',
        '',
        '    @define Int r0 = Int(math::random(0D, 2D))',
        '    @define Int spick = m1',
        '    @if r0 == 1',
        '        @var spick = m2',
        '    @elseif r0 == 2',
    ...
    for n, tip in enumerate(sim.entry_tips):
        ...
        sx, sz = SPAWN_OVERRIDE.get((b['x'], b['z']), (b['x'], b['z']))
        if n == 0:
            fl.append('    @if spick == 0')
        elif n < len(sim.entry_tips) - 1:
            fl.append('    @elseif spick == %d' % n)
        else:
            fl.append('    @else')
    ...
    fl += [
        ...
        '@fi',
        '',
        '# ---- normal path: random window %d-%d above the last bounce'
        % (FALL_SETBACK_MIN, FALL_SETBACK_MAX),
        '@define Int anchorY = lastBlock[player].getY()',
        '',
        '@define Int cnt = 0',
        '@for Int i in list::range(0, %d)' % nw,
        '    @if wys[i] >= anchorY + %d && wys[i] <= anchorY + %d'
        % (FALL_SETBACK_MIN, FALL_SETBACK_MAX),
        '        @var cnt += 1',
        '    @fi',
        '@done',
        ...
        '@define Int pick = -1',
        '@if cnt > 0',
        '    @define Int r = Int(math::random(0D, Double(cnt)))',
        '    @if r >= cnt',
        '        @var r = cnt - 1',
        '    @fi',
        '    @for Int j in list::range(0, %d)' % nw,
        '        @if pick == -1 && wys[j] >= anchorY + %d'
        ' && wys[j] <= anchorY + %d'
        % (FALL_SETBACK_MIN, FALL_SETBACK_MAX),
        '            @if r == 0',
        '                @var pick = j',
        ...
        '    @done',
        '@else',
        '    # band empty: closest window that still clears +%d, cap'
        % FALL_SETBACK_MIN,
        '    # ignored',
        '    @define Int bestD = 999999',
        '    @for Int k in list::range(0, %d)' % nw,
        '        @if wys[k] >= anchorY + %d && wys[k] - anchorY < bestD'
        % FALL_SETBACK_MIN,
        '            @var bestD = wys[k] - anchorY',
        '            @var pick = k',
        '        @fi',
        '    @done',
        '    # too near the top for a full +%d lift (nothing in the maze'
        % FALL_SETBACK_MIN,
        '    # is that far above an anchor up there): highest window at or',
        '    # above the anchor - the largest setback available, and still',
        '    # never a drop that would hand the player free progress down',
        '    @if pick == -1',
        '        @define Int bestY = -999999',
        '        @for Int k2 in list::range(0, %d)' % nw,
        '            @if wys[k2] >= anchorY && wys[k2] > bestY',
        '                @var bestY = wys[k2]',
        '                @var pick = k2',
        '            @fi',
        '        @done',
        '    @fi',
        '    # nothing at or above the anchor at all (cannot happen while',
        '    # windows reach y=%d, kept as pure safety): nearest below'
        % max(w[1] for w in wins),
        '    @if pick == -1',
        '        @var bestD = 999999',
        '        @for Int k3 in list::range(0, %d)' % nw,
        '            @if anchorY - wys[k3] < bestD',
        '                @var bestD = anchorY - wys[k3]',
        '                @var pick = k3',
        '            @fi',
        '        @done',
        '    @fi',
        ...
        '@var fallFx1(player)',
        '',
        '# 3 blocks above the window block, centered, keep facing',
        '@bypass /minecraft:tp {{player}} {{Double(wxs[pick]) + 0.5D}} '
        '{{wys[pick] + 3}} {{Double(wzs[pick]) + 0.5D}}',
        '',
        '@var fallFx2(player)',
    ]
    ...
    nms.append('    text(Int textNumber, Player player)')
    ...
    sl = ['START POSITIONS (seed %d) - one per entry corridor' % sim.seed,
          'A fall for a player who has NEVER bounced teleports them above',
          'a random one of the 3 of these furthest from the fall point;',
          'anyone with a lastBlock respawns at a splice window %d-%d'
          % (FALL_SETBACK_MIN, FALL_SETBACK_MAX),
          'blocks above it instead (see fall.msc).',
          '']
